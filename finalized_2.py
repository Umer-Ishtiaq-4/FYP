...
def draw_floor_plan(image, curr_box, texture_image, original_size, new_size):
    wall_thickness =  1
    wall_symbol = 0.0
    ...
    texture_resized = texture_image.resize((x2_new - x1_new, y2_new - y1_new))
    texture_arr = np.array(texture_resized)
    texture_tensor = torch.from_numpy(texture_arr).permute(2, 0, 1).float() / 255.0
    image[:, y1_new:y2_new, x1_new:x2_new] = texture_tensor
...
def save_multiple_bboxes(bboxes_tensor, labels, filename, room_names):
    directory_path = "E:\wood\images"
    path = get_files_in_directory(directory_path)
    # bg_color = 9.0 / 13
    bg_color = 1.0
    original_size = (256, 256)
    new_size = (400, 400)  # Increase the image width to 384 pixels
    image = torch.ones((3, *new_size)) * bg_color

    # Draw each bounding box on the white background with the corresponding label
    textures = []  # Store texture paths for legend
    for bbox, label in zip(bboxes_tensor, labels):
        texture_path = path[random.randint(0, len(directory_path))]
        textures.append(texture_path)
        texture_image = Image.open(texture_path)
        image = draw_floor_plan(image, bbox, texture_image, original_size, new_size)

    # Normalize tensor back to range [0, 255]
    image = image.mul_(255)

    # Convert the tensor to a numpy array
    ndarr = image.add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()

    # Convert array to PIL Image for drawing text and legend
    im = Image.fromarray(ndarr).convert("RGB")
    
    # Draw legend
    draw = ImageDraw.Draw(im)
    font = ImageFont.truetype("arial.ttf", 15)  # Assuming Arial font is available; adjust as needed

    legend_x = 5
    legend_y = 5
    border_thickness = 2  # Adjust the thickness of the black border as needed

    for i, texture_path in enumerate(textures):
        # Draw a small image of the texture with a black border
        texture = Image.open(texture_path).resize((20, 20))
        texture_with_border = Image.new('RGB', (24, 24), (0, 0, 0))
        texture_with_border.paste(texture, (border_thickness, border_thickness))
        im.paste(texture_with_border, (legend_x, legend_y))

        # Draw the room name next to the texture in black color
        draw.text((legend_x + 30, legend_y), room_names[labels[i]], font=font, fill=(0, 0, 0))

        # Move to the next line in the legend
        legend_y += 40
